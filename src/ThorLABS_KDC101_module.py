...
class Application:
    def __init__(self):
        ...
        try:
            ...
            print(tuple)
            #[(31, 83815209)]                   # KDC101 Production ID=31 Production Serial Number=27000001
            # Assign KDC101
            ID = self.ID = int(tuple[0][0])          #  ID
            SN = self.SN = int(tuple[0][1])          #  SN
            print (ID,SN)
            self.apt_KDC101 = apt.Motor(SN)     # KDC101 Production ID=31 Production Serial Number=27000001    old info TDC001=83815209
        except:
            print("missing Motor!")

    # Not use apt_KDC101.move_to(210.)  
    ...
    def Move_Abs(self,set_position):
        present_position = self.apt_KDC101.position      # check PP
        ...
        self.apt_KDC101.move_by(diff) 
        time.sleep(wait_time)
        present_position = self.apt_KDC101.position 
        return present_position
    
    def Move_Rel(self,diff):
        self.apt_KDC101.move_by(diff)   # diff<0.5deg, then sleep time=4.0sec
        time.sleep(4.0)                 # diff<0.5deg, then sleep time=4.0sec
        present_position = self.apt_KDC101.position    # check PP
        return present_position
    
    def Move_Home(self):
        self.apt_KDC101.move_home(True)
        present_position = self.apt_KDC101.position    # check PP
        return present_position
...
if __name__ == '__main__':
    KDC101 = Application()
'''        
    pp = KDC101.Move_Abs(5.0)
    print(pp)
    pp = KDC101.Move_Rel(-0.3)
    print(pp)
    pp = KDC101.Move_Home()
    print(pp)
    pp = KDC101.P_P()
...
# usage example
# import ThorLABS_KDC101_module
#         self.KDC101       = ThorLABS_KDC101_module.Application()
#         Present_Position  = self.KDC101.Jog_P05()
